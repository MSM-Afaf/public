...
mouth_normel = c.create_line(170, 250, 200, 272, 230, 250,smooth=1, width=2, state=NORMAL)
...
def show_sad(event):
    if (20 <= event.x <= 350) and (20 <= event.y <= 350):
        ...
        c.itemconfigure(mouth_normel, state=HIDDEN)
...
def hide_sad(event):
    ...
    c.itemconfigure(mouth_normel, state=NORMAL)
    c.itemconfigure(mouth_happy, state=HIDDEN)
    return
c.bind('<Motion>', show_sad)
c.bind('<Leave>', hide_sad)
root.mainloop()
